...
// GetActiveMenus retrieves all active menus ordered by order_index
...
// GetUserMenus retrieves all menus for a user through their role
func (r *repo) GetUserMenus(userId string) (ret []domainmenu.MenuItem, err error) {
	// First check if user has role_id
	var user struct {
		RoleId *string
		Role   string
	}
	...
	if user.RoleId != nil && *user.RoleId != "" {
		...
		// Fallback: use role string to get menus
		query := `
			SELECT DISTINCT m.*
			FROM menu_items m
			INNER JOIN role_menus rm ON m.id = rm.menu_item_id
